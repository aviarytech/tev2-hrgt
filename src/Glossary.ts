...
export class Glossary {
  ...
  private getSafMap(safURL: string): SAF {
    ...
    try {
      ...
      const requiredProperties: ScopeProperty[] = [
        "scopetag",
        "scopedir",
        "curatedir",
        "defaultvsn",
      ...
      if (missingProperties.length > 0) {
        log.error(
          `E002 Missing required property in SAF at '${safURL}': '${missingProperties.join(
            "', '"
          )}'`
        );
        process.exit(1);
      ...
      log.error(
        `E004 An error occurred while attempting to load the SAF at '${safURL}':`,
        err
      );
      process.exit(1);
    }
  ...
  public async getMrgMap(mrgURL: string): Promise<MRG> {
    ...
    try {
      ...
      const requiredProperties: TerminologyProperty[] = [
        "scopetag",
        "scopedir",
        "curatedir",
        "vsntag",
      ...
      const missingProperties = requiredProperties.filter(
        (prop) => !terminology[prop]
      );

      if (missingProperties.length > 0) {
        log.error(
          `E003 Missing required property in MRG at '${mrgURL}': '${missingProperties.join(
            "', '"
          )}'`
        );
        process.exit(1);
      ...
      for (const entry of (await mrg).entries) {
        ...
        const missingProperties = requiredEntryProperties.filter(
          (prop) => !entry[prop]
        );
  ...
  public async populateRuntime(mrg: MRG, filename: string): Promise<Output> {
    try {
      ...
      for (const entry of mrgEntries) {
        const alternatives = entry.formPhrases
          ? entry.formPhrases.split(",").map((t) => t.trim())
          : [];
        ...
        for (const alternative of alternatives) {
          const generatedAlternatives = applyMacroReplacements(
            alternative,
            regexMap
          );
      ...
      log.error(
        `E006 An error occurred while attempting to process the MRG at '${filename}':`,
        err
      );
      throw err;
    } finally {
